  ...
  try {
    ...
    fs.createReadStream(filePath)
      ...
      .on('end', async () => {
        try {
          ...
          if (pythonError.code === 'ECONNREFUSED') {
            res.status(503).json({ 
              success: false,
              error: "El servidor de análisis no está disponible. Asegúrate de que el servidor Python esté ejecutándose en el puerto 5000." 
            });
          } else {
            res.status(500).json({ 
              success: false,
              error: pythonError.response?.data?.error || pythonError.message || "Error procesando el análisis EEG" 
            });
          }
        }
      })
      .on('error', (csvError) => {
        ...
        res.status(400).json({ success: false, error: "Error procesando el archivo CSV: " + csvError.message });
      });
